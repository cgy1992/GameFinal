...
class VehicleApiUtils
{
	public:
		...
		static void HK_CALL updateCamera(hkDemoEnvironment* environment,
										...
										hkReal nearPlane = 0.5f,
										hkReal farPlane = 1000.f
										);
		*/

		/// add by myself
		static void HK_CALL calcWheelsRotation(hkpVehicleInstance& vehicle, hkQuaternion rots[]);

			/// Steer the vehicle.
		static void HK_CALL steer(
							hkReal& inputX,
							hkReal& inputY,
							hkpVehicleInstance& vehicle,
							hkReal timestep);
