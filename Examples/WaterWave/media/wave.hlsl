...
float4 PS(DomainOut pin) : SV_TARGET
{
	...
	color += GF_AMBIENT * GF_MTRL_AMBIENT + GF_MTRL_EMISSIVE;
	...
	if(dist < gPointLight.Range)
	{
		...
		if(diffAtten > 0)
		{
			...
			color += (gPointLight.Diffuse * GF_MTRL_DIFFUSE * diffAtten) * distAtten;
			SpecColor = gPointLight.Specular * GF_MTRL_SPECULAR * pow(specAtten, GF_MTRL_SPECULAR.w) * distAtten;
		}
	}
