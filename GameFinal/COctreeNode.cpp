...
namespace gf
{
	...
	void COctreeNode::intersectRay(FXMVECTOR Origin, FXMVECTOR Direction, f32* pDist,
		ISceneNode** ppSceneNode, u32 nodeType) const
	{
		f32 dist;
		...
		if (nodeType | ENT_LIGHT_NODE)
		{
			for (auto it = mStaticLightNodes.begin(); it != mStaticLightNodes.end(); it++)
			{
				if ((*it)->intersectRay(Origin, Direction, &dist))
				{
					if (dist < *pDist)
					{
						*pDist = dist;
		...
		{
			if (mChildrenNodes[i])
				mChildrenNodes[i]->intersectRay(Origin, Direction, pDist, ppSceneNode, nodeType);
		}
	}
